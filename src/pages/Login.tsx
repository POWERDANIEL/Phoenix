        ...
        <div className='flex flex-col p-4'>
          ...
          <div className='flex flex-col gap-2 max-w-[480px] w-full mx-auto'>
            ...
            <form onSubmit={handleSubmit} className='grid grid-cols-1 gap-4'>
              <TextField type='email' name='email' label='Email' placeholder='Email' required autoFocus />
              <TextField type='password' name='password' label='Password' placeholder='Enter your password' required />
              <div className='text-right'>
                <Link href='/reset-link' className='link text-labelLarge inline-block'>
                  Forgot password?
                </Link>
              </div>
              <Button type='submit' disabled={loading}>
                {loading ? <CircularProgress size='small' /> : 'Sign in'}
              </Button>
            </form>
            <p className='text-bodyMedium text-light-onSurfaceVariant dark:text-dark-onSurfaceVariant text-center mt-4'>
              Don&apos;t have an account?
              <Link href='/register' className='link text-labelLarge inline-block ms-1 text-light-onSurface dark:text-dark-onSurface'>
                Create an account
              </Link>
            </p>
          </div>
